...
- [X] T022 [P] [US1] Create tests/Unit/Actions/Media/GenerateVariantsActionTest.php - test 8 variants, skip upscaling, aspect ratio preservation, WebP conversion → @test-automator, @laravel-specialist
- [X] T023 [P] [US1] Create tests/Unit/Actions/Media/UploadToS3ActionTest.php - test success, retry logic (3 attempts with 1s/2s/4s delays), rollback on failure → @test-automator, @laravel-specialist
- [X] T024 [P] [US1] Create tests/Feature/MediaUploadTest.php - test complete upload flow, state transitions, variant generation → @test-automator, @laravel-specialist
- [X] T025 [P] [US1] Create tests/Feature/MediaStateTransitionTest.php - test uploading→processing→ready and uploading→processing→failed paths → @test-automator, @laravel-specialist
- [X] T026 [P] [US1] Create tests/Feature/Jobs/ProcessMediaVariantsJobTest.php - test async processing, S3 upload, variant record creation → @test-automator, @laravel-specialist
...
- [X] T028 [P] [US1] Create app/Actions/Media/SanitizeFilenameAction.php - lowercase, hyphens, remove special chars, append 8-char nanoid → @laravel-specialist, @php-pro
...
- [X] T033 [US1] Create app/Services/Media/MediaUploadService.php - orchestrate upload flow: validate, sanitize, extract metadata, create asset, dispatch job → @laravel-specialist
- [X] T034 [US1] Verify all US1 tests pass with ./vendor/bin/sail artisan test --filter=MediaUpload → @test-automator
...
- [X] T037 [US2] Update app/Services/Media/MediaUploadService.php - add SVG handling path: validate, sanitize, upload directly without variant generation → @laravel-specialist
- [X] T038 [US2] Verify all US2 tests pass with ./vendor/bin/sail artisan test --filter=SanitizeSvg → @test-automator
...
- [X] T040 [US3] Add feature test in tests/Feature/MediaUploadTest.php for video upload flow → @test-automator, @laravel-specialist
...
- [X] T053 [US5] Verify all US5 tests pass with ./vendor/bin/sail artisan test --filter=MediaFallback → @test-automator
...
- [X] T068 [US8] Add purge logic for soft-deleted assets older than 30 days to CleanupFailedMediaJob → @laravel-specialist
- [X] T069 [US8] Register scheduled jobs in routes/console.php: CleanupFailedMediaJob daily at 3:00, SyncOrphanedFilesJob weekly Sunday at 4:00 → @laravel-specialist
- [X] T070 [US8] Verify all US8 tests pass with ./vendor/bin/sail artisan test --filter=Cleanup → @test-automator
