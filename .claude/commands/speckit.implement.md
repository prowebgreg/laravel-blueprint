...
6. **Execute task using assigned subagent(s)** (MANDATORY):
   ...
   g. **Commit changes** (checkpoint after each task):
      Execute a checkpoint commit for the completed task:
      ...
      3. **Create commit message**:
         - First line: `feat(T00X): [task description summary]` (50-72 chars)
         - Use imperative mood (e.g., "Add feature" not "Added feature")
         - Body: Include:
           - What changes were made (list of key files)
           - Task ID and description from tasks.md
           - Any important technical details
         - Footer: Include Claude Code co-author attribution

      4. **Execute commit**:
         ```bash
         git commit -m "$(cat <<'EOF'
         feat(T00X): [concise task summary]

         Implements task T00X from tasks.md:
         - [Task description]

         Changes:
         - [List of key files changed/created]
      ...
      5. **Verify commit**: Run `git log -1 --oneline` to confirm
...
- **Auto-commit**: Each completed task is automatically committed with a descriptive message
- Commit format: `feat(T00X): [task summary]` with task details in the body
- If you need to skip the commit step, manually mark the task complete in tasks.md
