...
Use the `webapp-testing` skill for browser-based accessibility verification. Critical for testing real assistive technology interactions. Invoke when you need to:

- Test keyboard navigation flows in a live browser
- Verify focus management and tab order
- Capture screenshots for visual accessibility review
- Test ARIA live regions and dynamic content
- Validate screen reader announcement order

To use: `Skill: webapp-testing`

The skill provides Playwright automation for accessibility testing, enabling DOM inspection and interaction testing.

### html5-expert

Use the `html5-expert` skill for validating semantic HTML and accessibility patterns. This is a core tool for accessibility testing. Invoke when:

- Auditing semantic HTML structure (landmark elements, heading hierarchy)
- Validating ARIA usage against semantic HTML alternatives
- Checking content model compliance for proper element nesting
- Verifying form accessibility patterns (labels, fieldsets, error messages)
- Reviewing table markup for screen reader compatibility
- Ensuring semantic elements are used instead of ARIA role overrides
