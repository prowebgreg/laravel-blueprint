                    ...
                    Forms\Components\Section::make()
                        ->schema([
                            ...
                            Forms\Components\TextInput::make('slug')
                                ...
                                ->required()
                                ->maxLength(255)
                                ->disabled(fn (?BlogPost $record): bool => $record !== null)
                                ->dehydrated(fn (?BlogPost $record): bool => $record === null)
                                ->helperText(fn (?BlogPost $record): ?string => $record !== null ? 'Slug cannot be changed after creation' : null),

                            Forms\Components\Placeholder::make('created_at')
                                ->label('Created')
                                ->content(fn (?BlogPost $record): string => $record?->created_at?->diffForHumans() ?? '-')
                                ->visible(fn (?BlogPost $record): bool => $record !== null),
                            ...
                            Forms\Components\Placeholder::make('id')
                                ->label('Record ID')
                                ->content(fn (?BlogPost $record): string => (string) ($record?->id ?? '-'))
                                ->visible(fn (?BlogPost $record): bool => $record !== null),
                        ])
                        ->grow(false),
                ])->from('md'),
